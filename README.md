...
- [Ogni capacità si raggiunge da un gate, e arriva spiegata](#ogni-capacità-si-raggiunge-da-un-gate-e-arriva-spiegata)
...
La chiusura sta in un comando solo, `tools/chiudi-sessione.ps1` con la variante `.sh`, che l'utente lancia dal proprio terminale dopo aver chiuso Claude. Su Windows `Installa-Comandi.ps1` installa `chiudi` nel profilo PowerShell; su Linux e macOS `bash .claude/templates/tools/installa-chiudi.sh` installa la stessa funzione nel profilo Bash o Zsh, con ricerca dinamica dello script nel repository corrente. Esegue i controlli istanziati nel progetto e si ferma prima del commit se uno fallisce; prende il messaggio che l'agente ha preparato in `_notes/COMMIT-MSG.txt` e chiede conferma prima di committare e pushare; registra l'impronta solo dopo aver verificato sul remoto `origin` che il commit sia arrivato al ramo corrente; ed esegue il wipe degli account quando nessuna sessione Claude Code è ancora aperta. Commit e push restano un gesto dell'utente, perché lo script lo lancia l'utente e il commit parte solo dopo la sua conferma.
...
Il template non impone un modo di separare test e produzione: lo fa scegliere e lo fa capire. All'inizializzazione e a ogni tornata di allineamento la skill `separazione-ambienti` conduce un gate: dichiara prima i fatti che osserva nel repository e, su richiesta, sulla macchina di esercizio, cioè dove gira la produzione, file di composizione, configurazioni del reverse proxy, pipeline, file d'esempio delle variabili e branch remote; pone soltanto le domande a cui i fatti non rispondono; e per ogni asse propone una forma spiegando che cosa significa sceglierla invece dell'alternativa più vicina, che cosa richiede, che cosa costa e come fallisce, con le fonti. In un progetto esistente il modello c'è quasi sempre già, anche se nessuno l'ha scritto, e il gate lo riconosce e lo documenta invece di sostituirlo. L'esito va nella sezione "Modello di separazione" di `context/deployment.md`, con lo stato in esercizio o previsto di ogni ambiente e i rischi da presidiare, e come ADR in `memory/decisions.md`; guida e registro delle fonti si istanziano nel progetto sotto `docs/separazione-ambienti/`.

La scelta si divide in quattro assi indipendenti, perché confonderli è l'errore più comune. L'asse R dice dove girano gli ambienti: solo produzione con backup e istantanee per un'applicazione di terze parti, gemelli sulla stessa macchina dietro il reverse proxy, stack di container isolati sulla stessa macchina sempre accesi con una composizione parametrizzata oppure accesi a richiesta, macchina di staging separata con produzione immutabile da immagini, progetti gemelli su una piattaforma gestita con emulatori e anteprime per richiesta di modifica. L'asse P dice come passa il codice: una branch stabile con rami brevi e richieste di modifica, una branch di staging condivisa, una branch per ambiente, che la letteratura chiama un anti-pattern e che il template documenta solo dove esiste, e il trunk-based development con interruttori di funzionalità, non ancora osservato nei progetti. L'asse D dice da dove vengono i dati di prova: vuoti o con un seme, propri dell'ambiente di sviluppo con promozione controllata, copia della produzione resa innocua, istantanea su richiesta. L'asse L dice come stanno i sorgenti sulla macchina di chi sviluppa: un albero solo, più alberi di lavoro con `git worktree`, cloni indipendenti per ambiente.

Tre oggetti tengono la capacità, ciascuno con un ruolo. La regola `.claude/rules/separazione-ambienti.md` è la norma breve: il catalogo, sei rischi trasversali osservati su modelli diversi, primo fra tutti l'ambiente di prova esposto alla rete accanto alla produzione trovato su quattro macchine, e il gate. La skill `.claude/skills/separazione-ambienti/` è la procedura dell'interazione. Il pacchetto `.claude/templates/separazione-ambienti/` è la conoscenza: `GUIDA.md` descrive ogni forma con gli stessi campi, così che due forme si confrontino campo per campo, con estratti annotati e con le pratiche trasversali che correggono i rischi senza cambiare forma, cioè costruire una volta e promuovere lo stesso artefatto, configurazione separata dal codice, parità fra ambienti, ambienti effimeri completi, rilasci blu e verde, canarini e al buio, migrazioni in due tempi, infrastruttura dichiarata e segreti per ambiente; `FONTI.md` registra i nove casi osservati, descritti per archetipo, e trentasette fonti pubblicate lette il 2026-09-23, ciascuna con indirizzo, data dichiarata, licenza, stato di lettura, citazioni letterali e punti del template in cui è usata; `esempi/` contiene due composizioni verificate con `docker compose config`, il modello delle variabili per ambiente e lo scheletro dell'ADR. Le pratiche raccomandate dalla letteratura e non ancora viste in un progetto sono dichiarate come tali, e passano fra le osservate quando un progetto le adotta.

Gli alberi di lavoro sono una scelta fra le altre, e hanno una conseguenza che la regola `.claude/rules/alberi-di-lavoro.md` governa: la memoria versionata vale per la branch su cui è scritta, e un albero aperto su una branch indietro riceve uno snapshot e un registro delle decisioni ben formati e vecchi. La memoria si legge allora dall'albero della branch più avanti, per percorso assoluto, senza copiarla né fonderla, e `tools/verifica-ripresa.py` segnala all'apertura ogni altro albero la cui branch abbia fatto avanzare `.claude/memory/`, nominandone il percorso. Con una branch di staging condivisa lo stesso fenomeno si presenta in forma più lieve, e lo snapshot `memory/index.md` porta per questo la riga degli ambienti con i commit di produzione e di staging sul remoto. Il principio sta nella sezione 22 di `PROJECT-SYSTEM.md`.

## Ogni capacità si raggiunge da un gate, e arriva spiegata

Una capacità del template esiste, per chi avvia o allinea un progetto, soltanto se l'inizializzazione o l'allineamento gliela propongono, e gliela propongono in modo utile soltanto se spiegano quando conviene e che cosa comporta rispetto alle alternative. La sezione 23 di `PROJECT-SYSTEM.md` ne fa un principio con una forma in tre parti, norma, procedura e conoscenza con le fonti, che la separazione fra test e produzione mostra per intero. Il presidio è doppio: `check-catalogo.py` verifica che ogni pacchetto abbia una voce nel catalogo attraversato dal gate dei pacchetti, e `check-raggiungibilita.py` verifica che ogni regola e ogni skill sia nominata da almeno un punto d'ingresso, salvo le capacità sempre attive, dichiarate in un elenco con il motivo di ciascuna. Alla prima corsa ha trovato una lacuna reale, la skill `studio-didattico` che nessun gate proponeva, ora portata dal pacchetto `documentazione-didattica`. Nessuno dei due controlli giudica la qualità della spiegazione, e lo dichiarano.
...
template-claude-developing/
  ...
  .claude/
    ...
    skills/
      ...
      separazione-ambienti/      gate della separazione fra test e produzione, spiegato dalla guida del pacchetto
...
- `separazione-ambienti` - paradigmi di separazione fra test e produzione, spiegati e con le fonti: [.claude/templates/separazione-ambienti/README.md](.claude/templates/separazione-ambienti/README.md)
...
- Fonti della separazione fra test e produzione, registrate una per una con data, licenza, stato di lettura e citazioni in `.claude/templates/separazione-ambienti/FONTI.md`. Le principali: The Twelve-Factor App di Adam Wiggins (https://12factor.net/); Patterns for Managing Source Code Branches di Martin Fowler (https://martinfowler.com/articles/branching-patterns.html); A successful Git branching model di Vincent Driessen con la nota del 2020 (https://nvie.com/posts/a-successful-git-branching-model/); trunkbaseddevelopment.com di Paul Hammant (https://trunkbaseddevelopment.com/); Feature Toggles di Pete Hodgson (https://martinfowler.com/articles/feature-toggles.html); Continuous Delivery di Jez Humble (https://continuousdelivery.com/); le capacità DORA (https://dora.dev/capabilities/); Canarying Releases del Google SRE Workbook (https://sre.google/workbook/canarying-releases/); Evolutionary Database Design di Pramod Sadalage e Martin Fowler (https://martinfowler.com/articles/evodb.html); OWASP Secrets Management Cheat Sheet (https://cheatsheetseries.owasp.org/cheatsheets/Secrets_Management_Cheat_Sheet.html); la documentazione di Docker Compose, git-worktree, Firebase, Vercel, Odoo, Proxmox VE, Next.js e nginx
