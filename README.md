...
La stessa regola di stile vincola anche la forma dei sorgenti `.md`: ogni paragrafo sta su una riga sorgente continua, senza a capo manuali a meta' frase, perche' l'avvolgimento a video e' compito dell'editor e non del file. Il beneficio e' sui diff, che tornano a segnare esattamente i paragrafi cambiati invece di ri-avvolgere righe che nessuno ha toccato, e sulla ricerca testuale per frase, che con il wrap manuale non trova nulla.

Il pacchetto opzionale `md-unwrap` (vedi `.claude/templates/md-unwrap/`) attua la convenzione meccanicamente con un formatter a diff minimo: unisce le righe di continuazione di paragrafi, voci di elenco e citazioni, e non normalizza nient'altro, lasciando identici marcatori di lista, tabelle, stili di titolo ed escaping. Prima di scrivere un file lo verifica due volte, con un'invariante interna sul flusso dei caratteri e, quando `markdown-it-py` e' disponibile, con un oracolo che rende in HTML originale e output e rifiuta la scrittura se il rendering normalizzato cambierebbe. Ha una CLI batch su file, glob e cartelle con `--check` per il gate non distruttivo e `--diff` per l'anteprima, ed e' in Python 3 senza dipendenze obbligatorie. L'alternativa esterna `prettier --prose-wrap never` risolve lo stesso problema ma normalizza anche liste, tabelle, titoli ed escaping nello stesso passaggio, il che su documentazione scritta a mano produce un diff in cui la modifica voluta e' indistinguibile dal resto: il confronto completo, con i pro e i contro delle due strade, e' nel README del pacchetto.
...
Per i progetti che ragionano o scrivono a partire da un corpus di fonti proprie dell'utente, il pacchetto opzionale `notebooklm-bridge` scaffolda il metodo di ricerca fondata che accoppia NotebookLM come motore di recupero ancorato alle fonti e Claude come motore di ragionamento e scrittura, nel loop a quattro passi degli screenshot di origine: NotebookLM sintetizza le fonti con citazioni, Claude ragiona sulla sintesi confrontando angolazioni e cercando contraddizioni, rimanda a NotebookLM cio che va verificato contro le citazioni, e infine scrive la versione finale. Il pacchetto e' costruito tassativamente attorno al piano gratuito di NotebookLM, accessibile solo da browser: non usa e non propone l'API Enterprise a pagamento, che sarebbe l'unico endpoint ufficiale ma richiede credenziali aziendali e un progetto Google Cloud a pagamento.

Il valore sta all'incrocio dei due assi, risparmio di token e ricerca, serviti dallo stesso meccanismo: il corpus resta dentro NotebookLM, fuori dalla finestra di contesto di Claude, e in conversazione entra solo la sintesi densa e citata, poche migliaia di token invece del corpus intero. E' la stessa disclosure progressiva del pacchetto `doc-ingest`, con il Livello 1 prodotto da un motore di recupero esterno che ancora ogni affermazione a una fonte. L'accesso avviene in due modi: quello manuale, di default e pienamente conforme, in cui l'utente carica le fonti e incolla la sintesi a Claude; e quello assistito, opt-in, in cui Claude pilota il browser tramite un MCP della community (`notebooklm-mcp`), che riusa un profilo Chrome con l'account Google connesso ma vive in una zona grigia dei termini di servizio, per cui non e' mai il default. Prima del loop, lo strumento `tools/notebooklm-check.ps1` (o `.sh`) verifica quali account Google e quali browser sono disponibili per NotebookLM su questa macchina, e poiche' lo stato di login non e' accertabile dall'esterno del browser, la conferma finale e' uno screenshot di `notebooklm.google.com` loggato, letto secondo la regola degli screenshot. Il dettaglio completo, coi crediti e la mappa di istanziazione, e' in `.claude/templates/notebooklm-bridge/README.md`. Deriva da screenshot di un metodo pubblico condiviso dall'utente, non ancora validato su un progetto reale.
...
Ogni pacchetto a cartella porta con se' un proprio `README.md` di istanziazione sotto `.claude/templates/<nome>/`, che ne spiega il funzionamento su due piani, concettuale e operativo, con la mappa di istanziazione e i crediti agli strumenti open source. Questo indice li raccoglie tutti, ed e' il punto da cui aggiungere la riga quando nasce un pacchetto nuovo: il README di radice indicizza i README dei pacchetti, non li incorpora. I pacchetti che non sono cartelle, gli strumenti esterni e i server MCP, non hanno un README dedicato e vivono come righe del catalogo `PACKAGES.md`.
...
- `Prettier`, formatter multi-linguaggio la cui opzione `--prose-wrap never` risolve lo stesso problema di `md-unwrap` normalizzando pero' anche liste, tabelle, titoli ed escaping (licenza MIT); documentato come alternativa nel README del pacchetto, non adottato come default perche' non e' a diff minimo: https://github.com/prettier/prettier
...
- NotebookLM di Google, motore di recupero ancorato alle fonti alla base del pacchetto `notebooklm-bridge`; usato nel piano gratuito accessibile da browser (https://notebooklm.google.com), che non espone un'API pubblica per consumatori. L'unica API ufficiale e' quella Enterprise su Google Cloud, a pagamento e non usata dal pacchetto
