...
Non leggere mai una fonte voluminosa per intero se non serve: estrarne il segnale a fette, coerentemente con l'ingestione documenti del sistema. Non modificare `knowledge/sources/`: è append-only. Non eseguire `git add`, `commit` o `push`: le operazioni git restano all'utente. Non inventare collegamenti o contenuti non presenti nelle fonti.
