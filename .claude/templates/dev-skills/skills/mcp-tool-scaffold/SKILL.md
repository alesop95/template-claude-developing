...
3. Scrivi le description per il modello che le legge: la description del tool e dei parametri è ciò che un LLM usa per decidere se e come invocarlo, quindi dichiara cosa fa, quando usarlo e quali effetti collaterali ha, non solo cosa significa tecnicamente.
4. Genera i tre test minimi: input valido, input non valido, errore a runtime.

## Best practice

Errori strutturati e informativi, mai stack trace grezzi verso il client. Idempotenza dove possibile, ed effetti collaterali sempre dichiarati nella description. Il nome e lo schema di un tool pubblicato sono un contratto: cambiarli è una breaking change da registrare in `memory/decisions.md`.
