...
## 22. Ambienti separati in alberi di lavoro, e la memoria che resta una

Generalizzato da un progetto istanziato che teneva tre alberi di lavoro sulla stessa macchina, uno per la vetrina su `staging`, uno per un lavoro grafico su una branch tematica e uno per una funzionalità indipendente. La forma di sviluppo è quella che il template raccomanda quando servono ambienti di test e di produzione separati: invece di cambiare branch avanti e indietro in una sola cartella, ogni ambiente ha il proprio albero, con la propria branch in uscita, il proprio `.env`, la propria porta e la propria base dati locale, e la tabella degli alberi sta nella scheda `context/deployment.md`. La procedura, i vincoli di git che non si leggono dal comando e le contromisure stanno nella regola `rules/alberi-di-lavoro.md`, che è la fonte normativa e che qui non si duplica.

Ciò che merita di stare in questo documento è il principio, perché riguarda il sistema e non la tecnica. Un sistema che versiona la memoria nel repository ottiene che la memoria viaggi con il codice, e paga questa proprietà con un'altra che è la stessa guardata dall'altro lato: la memoria vale per la branch su cui è scritta, non per il progetto. Con un albero solo lo scarto non si vede; con due il progetto ha due memorie senza che nessuno lo abbia deciso, e una sessione aperta sulla branch indietro legge per prima, come la sezione 12 le chiede, uno snapshot ben formato, coerente e vecchio. Nel caso osservato mancavano sedici decisioni su trentadue, fra cui quella che istituiva il modello a più alberi.

La risposta non è copiare la memoria negli altri alberi, che produce due memorie divergenti, né fonderla, che porta nella branch commit che non le appartengono e ne rende illeggibile la pull request: la memoria si legge dall'albero della branch più avanti, per percorso assoluto, e la tabella degli alberi dichiara quale sia. Il presidio, secondo la sezione 17, è doppio: l'avviso in testa al file di ripresa di ogni albero secondario, che essendo non versionato è il solo posto che non eredita il tranello, e il confronto con gli altri alberi dentro `tools/verifica-ripresa.py`, che trasforma in un avviso la cecità del motore di riconciliazione verso una memoria appartenente a un'altra branch.

La generalizzazione vale anche con un albero solo, ed è una regola di redazione: ogni documento che descrive uno stato invece di una regola, cioè snapshot, registro delle decisioni, lavoro in corso, roadmap e work-log, dichiara in testa a quale commit e a quale branch si riferisce. I documenti di regole degradano molto più lentamente di quelli di stato, e la prudenza giusta è proporzionata al ritmo con cui un documento invecchia, non uniforme.
