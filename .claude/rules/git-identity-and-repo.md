...
Profilo di lavoro: alias SSH `github-corp`, chiave `id_ed25519_corp`, identità `user.name` <user-account1> e `user.email` <email-account1>, organizzazione <org-lavoro>. E il profilo con cui questo repository e già configurato.

Profilo personale: alias SSH `github-personal`, chiave `id_ed25519_personal`, identità `user.name` <user-personale> e `user.email` <email-personale>, utente GitHub <user-personale>.
